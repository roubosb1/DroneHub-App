...
// ── Job timezone: derived from the shoot's location, not ours ────────────────
const US_STATE_TZ={
...
const US_MARKET_TZ={new_york:'America/New_York',texas:'America/Chicago',arizona:'America/Phoenix',colorado:'America/Denver',other_us:'America/New_York',canada:'America/Toronto'};
function jobTimezone(job){
  // 1. state abbreviation in the address or name (", TX" / " TX 78723")
  const hay=((job.address||'')+' '+(job.name||'')).toUpperCase();
  const m=hay.match(/[,\s]([A-Z]{2})[\s,]+\d{5}/)||hay.match(/,\s*([A-Z]{2})(?:[\s,]|$)/);
  if(m&&US_STATE_TZ[m[1]]) return US_STATE_TZ[m[1]];
  // 2. the job's market
  if(job.market&&US_MARKET_TZ[job.market]) return US_MARKET_TZ[job.market];
