...
function renderNotificationPanel(){
  ...
  const bookingRows=requestedJobs.map(j=>{
    ...
    return `<div onclick="document.getElementById('notif-panel').style.display='none';showPane('sales');setTimeout(()=>{switchSalesView('jobs');},200)" style="padding:12px 16px;cursor:pointer;transition:background .15s" onmouseenter="this.style.background='rgba(245,166,35,.1)'" onmouseleave="this.style.background='transparent'">
  ...
  const notifRows=notifs.map(n=>{
    ...
    const navAction=n.type==='tax_update'?"showPane('finance')":(n.type==='rsvp_accept'||n.type==='rsvp_decline')?"showPane('calendar')":n.type==='timeoff_request'||n.type==='approval'&&n.text?.includes('time off')?"if(window.innerWidth<=768&&typeof mobShowProfile==='function'){mobShowProfile();setTimeout(()=>mobPfSwitchTab('timeoff'),150);}else{dskShowMyProfile();setTimeout(()=>dskPfSwitchTab('timeoff'),150);}":"showPane('social');setTimeout(()=>{setSocialSubTab('approvals');},200)";
